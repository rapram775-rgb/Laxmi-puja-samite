...
function App() {
  ...
  return <>
    ...
    <main>
      ...
      <Section id="committee" eyebrow="Leadership • Service • Teamwork" title="Our Committee" subtitle="Meet the ten principal committee members who lead, organize and serve the Shree Laxmi Puja Mahotsav."><div className="committeeGrid premiumCommitteeGrid">{c.committee.map(([role,name,image],i)=><article className="memberCard premiumMemberCard" key={i}><div className="memberPhoto">{image ? <img src={image} alt={`${name} — ${role}`} loading="lazy"/> : <Photo label="Photos"/>}</div><div className="memberIcon" aria-hidden="true">{i === 0 ? '♛' : i === 1 ? '◈' : i === 2 ? '✎' : i === 3 ? '▣' : i === 4 ? '♟' : i === 5 ? '⚑' : i === 6 ? '▦' : i === 7 ? '♥' : i === 8 ? '★' : '✦'}</div><div className="memberInfo"><strong className="memberRole">{role}</strong><h3>{name}</h3><span className="memberOrnament" aria-hidden="true"><i></i><b>✦</b><i></i></span></div></article>)}</div><div className="centerAction"><button className="btn outlineDark" type="button" onClick={() => setCommitteeOpen(true)}>सदस्यहरू <span className="arrow">→</span></button></div></Section>
